...
async def help_rules(ctx):
    ...
    embed.add_field(name=":black_small_square: No Copying of scripts via Discord or GitHub", value="(1)",inline=True)
    embed.add_field(name=":black_small_square: No illigal material should be added.", value="(2)",inline=True)
    embed.add_field(name=":black_small_square: No spamming of the bot.", value="(3)",inline=True)
    embed.add_field(name=":black_small_square: No raiding of the bot.", value="(4)",inline=True)
    embed.add_field(name="**Bot Server Rules**", value=" :link: ",inline=True)
    embed.add_field(name=":black_small_square: We do not responsiability for the damage within the server by the bot itself.", value="(1)",inline=True)
    embed.add_field(name=":black_small_square: We get reports of the user entering a command. We do not stalk people's chats.", value="(2)",inline=True)
    embed.add_field(name=":black_small_square: We have a High Security Script which means the users and servers are always secured.", value="(3)",inline=True)
    embed.set_footer(text="Requested by: " + author.name)
    await bot.send_message(author, embed=embed)
    embed = discord.Embed(description=" ", color=0xFFFF)
    embed.add_field(name=":white_check_mark: Success!", value="I've sent you a list of my ``Fun`` commands in your **Direct Messages**",inline=True)
