...
async def on_member_remove(user):
    channel = discord.utils.get(user.server, name='server-logs', type=discord.ChannelType.text)
    ...
    await bot.send_message(discord.Object(id='558973820686434352'),embed=embed)
...
async def helpcommands_test(ctx):
    ...
    embed.add_field(name="b.hug (ping a user)", value="Hug a user by a gif",inline=True)
    embed.add_field(name="b.kill (ping a user)", value="Kill a user by a gif",inline=True)
    embed.add_field(name="b.please (ping a user)", value="Beg a user by a gif",inline=True)
    embed.add_field(name="b.fortnite (ping a user)", value="Loser a user by a gif in Fortnite",inline=True)
    embed.add_field(name="b.thanks (ping a user)", value="Says Thank You in a gif",inline=True)
    embed.add_field(name="b.yourwelcome", value="Says Your Welcome in a gif",inline=True)
    embed.add_field(name="b.eightball (question)", value="Replyies to your question in a fun way",inline=True)
    embed.add_field(name="b.square (number)", value="Square a number in Maths",inline=True)
    embed.add_field(name="__Moderation Commands", value="Moderation Commands for specific permissions to use.",inline=True)
    embed.add_field(name="b.report (user & reason)", value="Reports a user. <a:alert:557215953839194123> Requries a channel called #mod-log",inline=True)
    embed.add_field(name="b.mute (user & reason)", value="Mutes a user. <a:alert:557215953839194123> Requries a role called Muted and above most roles. Decline all perms.",inline=True)
    embed.add_field(name="b.kick (user & reason)", value="Kicks a user and displays in the same channel the Moderator, User and Reason.",inline=True)
    embed.add_field(name="b.ban (user & reason)", value="Bans a user and displays the ban reason. ",inline=True)
    embed.add_field(name="b.bans", value="Administrator+, can see all of the banned members in the server.",inline=True)
    embed.add_field(name="b.clean", value="Manage Messages+, Clean a specific number of messages 98-.",inline=True)
...
async def updates(ctx):
    m1=await bot.say('<a:Graphiqloading:551796596350910475> Checking the lastest bot updates...')
    await asyncio.sleep(4)
    await bot.edit_message(m1,new_content='<a:alert:557215953839194123> New Updates!')
    await asyncio.sleep(4)
    await bot.edit_message(m1,new_content='<a:alert:557215953839194123> We believe at iCodeTM that we think its the best thing to do is shutdown Graphiq. We may change bot names but we think its for the best that we should shutdown Graphiq.')
    await asyncio.sleep(16)
    m2=await bot.edit_message(m1,new_content='<a:checkmark:552893708387876884> Thank you for understanding about this.')
    await asyncio.sleep(5)
    await bot.edit_message(m1,new_content='<a:Graphiqloading:551796596350910475>  Checking recent updates...')
    await asyncio.sleep(5)
